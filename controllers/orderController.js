...
exports.getAllOrder = catchAsync(async (req, res, next) => {
  // hàm catchAsync return ra hàm vô danh sau đó gán vào createTour, khi tạo mới tour thì hàm vô danh đó sẽ được gọi, cần tham số next để gọi đến global error handling middleware
...
exports.createOrder = catchAsync(async (req, res, next) => {
  ...
  await Seat.findByIdAndUpdate(
    { _id: req.body.seat },
    { status: "seat sold", description: "Bàn Đã Đặt" }
  );
  res.status(200).json({
    status: "success",
    data: {
...
exports.confirmOrder = catchAsync(async (req, res, next) => {
  ...
  if (
    comfirmOrder.seat.status == "seat sold" &&
    comfirmOrder.status == "Đã hủy"
  ) {
    return next(new Error("Bàn Đã Được Đặt Bởi Người Khác", 404));
  }
  await Seat.findByIdAndUpdate(
    { _id: comfirmOrder.seat.id },
    { status: "seat sold", description: "Bàn Đã Đặt" }
  ); // cập nhập lại chỗ ngồi
...
exports.confirmManymOrder = catchAsync(async (req, res, next) => {
  ...
  if (confirmAgainMany.length > 0) {
    return next(
      new Error(
        `Bàn của ${[...confirmAgainMany]} Đã Được Đặt Bởi Người Khác `,
        404
      )
    );
  }
  ...
  for (let i = 0; i <= arrOrder.length - 1; i++) {
    await Seat.findByIdAndUpdate(
      { _id: arrOrder[i].seat.id },
      { status: "seat sold", description: "Bàn Đã Đặt" }
    ); // cập nhập lại chỗ ngồi (many)
  }
...
exports.denyOrder = catchAsync(async (req, res, next) => {
  ...
  await Seat.findByIdAndUpdate(
    { _id: orderDeny.seat.id },
    { status: "seat", description: "Trống" }
  ); // cập nhập lại chỗ ngồi
