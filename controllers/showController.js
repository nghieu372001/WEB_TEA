...
exports.createShow = catchAsync(async (req, res, next) => {
  ...
  let arrSeat = [];
  for (let i = 1; i <= 15; i++) {
    arrSeat.push({ name: i, show: `${show._id}` });
  }
  //console.log(arrSeat)
...
exports.updateShow = catchAsync(async (req, res, next) => {
  ...
  const filterBody = filterObj(
    ...
    "content",
    "singer"
  ); //filterBody = { name: 'Sophie Louise Hart 1' }
  console.log(filterBody);
  if (req.file) {
    filterBody.imageShow = req.file.filename; //filterBody.photo vì upload.single('photo')
  }

  const show = await Show.findByIdAndUpdate(req.params.id, filterBody, {
    new: true,
    runValidators: true,
  });
  if (!show) {
    // nếu không tồn tại ID hợp lệ trong database thì sẽ tạo ra lỗi
